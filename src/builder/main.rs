...
use crate::{IOType, McSpaceName, McURI};
...
pub fn mcb_init_system_lib() {
    ...
    // ★ Regardless of whether the system library is successfully loaded from disk, ensure builtin primitive types are available
    register_builtin_primitives();
    debug!(target: "mcc::sysinit", "system lib init done (builtins registered)");
}

// ============================================================================
// ★ Builtin primitive type registration
// ============================================================================

/// List of 2-pin passive component types that need to be built-in registered
///
/// These types are used inline in .mc files via syntax like `CAP(1uF).Cap(...)` / `RES(10kΩ).Pullup(...)`,
/// and are basic building blocks of circuit descriptions. If the system library is not loaded correctly
/// (directory missing or library file incomplete), these types cannot be found by `mcb_get_cmie`, causing
/// all anonymous passive components in pass2 to degenerate into `@?` stubs —— no real component instances
/// are produced, cannot be registered in the downstream InstTable, and 75+ nets on the drawing side are
/// all dropped due to resolve failure.
const BUILTIN_TWOPIN_TYPES: &[&str] = &["CAP", "RES", "IND", "DIO", "DIO.ESD"];

/// Register built-in 2-pin passive component types to the workspace component table + mcc_blibs spacenames.
///
/// For each type name in `BUILTIN_TWOPIN_TYPES`, if there is no same-name definition in the workspace or
/// global table, programmatically create a minimal `McComponent` (containing only pin "1" and pin "2",
/// no parameters, functions, or attributes), and register it to `workspace::WORKSPACE.components` and
/// `mcc_blibs["mcode"].spacenames`.
///
/// Then `mcb_get_cmie("CAP")` can hit these definitions in step 1 (search system library spacenames) or
/// step 5 (look up the global table directly by name), and `instantiate_funccall` goes through the
/// `instantiate_component_construction` path to create real `@CAP_N` component instances (instead of
/// `@?CAP_N` stubs), thus fixing 75 dropped nets.
fn register_builtin_primitives() {
    use crate::core::basic::mc_param::McParamDeclares;
    use crate::core::component::mc_attr::McAttributes;
    use crate::core::component::mc_layout::McLayout;
    use crate::core::component::mc_pins::{McPin, McPinPort, McPins};
    use crate::core::component::McComponent;
    use crate::core::mc_func::McFunctions;
    use crate::core::mc_inst::McInstances;

    let builtin_uri: McURI = "builtin://mcode/primitives".to_string();

    // ★ Ensure mcc_blibs["mcode"] exists —— otherwise the `get_mut("mcode")` below cannot obtain it,
    //   spacenames not written, mcb_get_cmie step 1 not hit, built-ins effectively not registered.
    //   In the mcc_init path, mcb_init_system_lib has already created it first -> here it is idempotent no-op;
    //   in the mcc_init_no_lib path that directly calls this function, here is responsible for building the empty blib.
    if !global::mcc_blibs.borrow().contains_key("mcode") {
        global::mcc_blibs
            .borrow_mut()
            .insert("mcode".to_string(), McCode::new_empty());
    ...
    for &type_name in BUILTIN_TWOPIN_TYPES {
        let name_ids = McIds::from(type_name);
        let space_name = McSpaceName::new(&name_ids, builtin_uri.clone());

        // Skip already-existing definitions (disk system library or project file already defined)
        if workspace::WORKSPACE
            .components
            .borrow()
            .contains_key(&space_name)
        {
            continue;
        }
        // Also check by name (may have been registered with a different URI)
        let already_exists = workspace::WORKSPACE
            .components
            .borrow()
            .iter()
            .any(|e| e.key().ident.to_string() == type_name)
        ...
        // Create minimal 2-pin McPins
        let mut pins = McPins::new();
        let empty_values: Arc<Vec<crate::core::component::mc_attr::McAttrVal>> =
            Arc::new(Vec::new());
        // pin "1"
        pins.pins.insert(
            "1".to_string(),
            McPin {
                ...
                values: empty_values.clone(),
            },
        );
        pins.names_to_id
            .insert("1".to_string(), McPinPort::Single("1".to_string()));
        pins.pin_id_to_names
            .insert("1".to_string(), vec!["1".to_string()]);
        // pin "2"
        pins.pins.insert(
            "2".to_string(),
            McPin {
                iotype: IOType::None,
                id: "2".to_string(),
                names: vec!["2".to_string()],
                values: empty_values.clone(),
            },
        );
        pins.names_to_id
            .insert("2".to_string(), McPinPort::Single("2".to_string()));
        pins.pin_id_to_names
            .insert("2".to_string(), vec!["2".to_string()]);

        // Assemble McComponent
        // Note: McLayout::empty() is pub(super), cannot be called from the builder module,
        // construct directly (all fields are pub)
        let comp = McComponent {
            name: name_ids.clone(),
        ...
        // Register to workspace component table
        workspace::WORKSPACE
            .components
            .borrow()
            .insert(space_name.clone(), comp_arc);

        // Register to mcc_blibs["mcode"]'s spacenames (so mcb_get_cmie step 1 can hit)
        if let Some(mut blib) = global::mcc_blibs.borrow().get_mut("mcode") {
            blib.spacenames.insert(name_ids.clone(), space_name.clone());
        }

        debug!(
            target: "mcc::sysinit",
            type_name = type_name,
            "registered builtin 2-pin primitive"
        );
        registered += 1;
    }
    // Diagnostic: this line appearing = this function actually ran. If not seen in logs -> init path did not call it
    // (e.g. took mcc_init_no_lib without supplementary registration), built-ins missing -> inline passives all lost.
    eprintln!(
        "[builtins] register_builtin_primitives ran: {}/{} primitive type(s) newly registered",
        registered,
        BUILTIN_TWOPIN_TYPES.len()
    );
}

/// ★ Public wrapper: register built-in 2-pin primitives (CAP/RES/IND/DIO/DIO.ESD).
///
/// For init paths like `mcc_init_no_lib()` that **bypass** `mcb_init_system_lib()`,
/// ensures built-in passives are available regardless of whether the disk system library is loaded. Idempotent (safe to call repeatedly).
pub fn mcb_register_builtins() {
    register_builtin_primitives();
}
/// Get the number of loaded interfaces
