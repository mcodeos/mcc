...
use crate::semantic::validation::nets::{
    ...
    check_unwired_instances, check_unwired_pins, check_voltage_mismatch, NetCheckResult,
};
...
pub static FLAT_ERC_RULES: &[FlatErcRule] = &[
    ...
    declare_flat_erc_rule! {
        ...
        domain = Connectivity,
        family = None,
        doc = "An intentionally unconnected NC terminal appears on a net — both carriers report: the class-level `nc` direction word and the instance-site `@ncpin(…)` marker (U305, nc-design.md §4.3).",
        lock = "tests/flatten_net_check_diagnostics.rs",
        overridable = false,
    ...
    // BOM block binds (param-authoring-design.md section 4, U245; keyword `bom` since U290)
    declare_flat_erc_rule! {
        code = crate::errcodes::BOM_VALUE_NOT_DESCENDANT,
        name = "bom-value-descendant",
        title = "bom value is not a variant of the slot class",
        severity = Error,
        domain = Structure,
        family = None,
        doc = "A bom.mc value names a class that is not a `:` descendant of the slot's declared class.",
        lock = "tests/shard6/bom_binding.rs",
        overridable = false,
        owner = check_bom_value_descendant,
    },
    declare_flat_erc_rule! {
        code = crate::errcodes::BOM_KEY_NOT_SLOT,
        name = "bom-key-slot",
        title = "bom key does not designate an abstract slot",
        severity = Error,
        domain = Structure,
        family = None,
        doc = "A bom.mc key does not designate an abstract-declared instance.",
        lock = "tests/shard6/bom_binding.rs",
        overridable = false,
        owner = check_bom_key_slot,
    },
    ...
    // rail-contract-design.md §6.1 regulator gate (window batch) — a regulator
    // declares its operating pre-condition input_req; the supply window riding
    // its input net must sit inside it (S(input) ⊆ input_req). Resolved feeds
    // outside that window are Error; un-derivable feeds (NoSupply/Unresolved —
    // module-boundary, undeclared contention) are not adjudicated.
    declare_flat_erc_rule! {
        code = crate::errcodes::POWER_CONVERTER_GATE,
        name = "converter-gate-window",
        title = "regulator input window excludes the supply window on its input net",
        severity = Error,
        domain = Power,
        family = None,
        doc = "§6.1 regulator gate: a def writing the full regulator spec (input_req + output, ≥1 Snk input row, ≥1 Src row) claims its output post-condition holds only while S(input) ⊆ input_req. A Resolved supply window on its input net outside that declared window is an Error — the feed can push the regulator out of its operating window, so the output guarantee cannot be trusted. Nets whose window cannot be derived (NoSupply/Unresolved) are not adjudicated (rail-contract-design.md §6.1).",
        lock = "tests/power_intent_l1.rs",
        overridable = false,
        owner = check_converter_gate_window,
    },
    // rail-contract-design.md §6.3 sink req window (window batch) — a load
    // whose spec declares input_req states the supply window it accepts; the
    // Resolved supply window on its sink net must sit inside it. A regulator's
    // own input row is gated per net by 6023, not re-judged here.
    declare_flat_erc_rule! {
        code = crate::errcodes::POWER_SINK_WINDOW_MISMATCH,
        name = "sink-window-mismatch",
        title = "actual supply window escapes the load's declared acceptable window",
        severity = Error,
        domain = Power,
        family = None,
        doc = "§6.3 sink req window (E-PWR-001 window step): a load declaring spec input_req tolerates supply only inside that window. A Resolved supply window on the sink's net outside it over- or under-volts the load and is an Error. Nets whose window cannot be derived, and regulator input rows (6023's per-net gate), stay unjudged (rail-contract-design.md §6.3).",
        lock = "tests/power_intent_l1.rs",
        overridable = false,
        owner = check_sink_window_mismatch,
    },
    // rail-contract-design.md §6.1 partial-spec advisory (window batch) — a def
    // with a psrc/psbi output row whose spec block writes only one side of the
    // Hoare triple (input_req / output) cannot be gated by 6023/6024. Advisory
    // Info, decl-local: the written side still decodes, the missing side is
    // named. A pure load (no output row) legitimately writes input_req alone.
    declare_flat_erc_rule! {
        code = crate::errcodes::POWER_CONVERTER_SPEC_INCOMPLETE,
        name = "converter-spec-incomplete",
        title = "spec block on a power-output component declares only one of input_req / output",
        severity = Info,
        domain = Power,
        family = None,
        doc = "§6.1 partial-spec advisory: a component with a psrc/psbi output row writes a spec block but declares only one of input_req or output. The regulator's gate (6023) needs both windows of the Hoare triple; a one-sided write decodes but cannot be gated. The written side still holds (an output-only regulator is treated as guaranteeing that output; an input_req-only one as an un-gated feed); add the missing side. A pure load with no output row legitimately writes input_req alone and is never flagged (rail-contract-design.md §6.1).",
        lock = "tests/power_intent_l1.rs",
        overridable = false,
        owner = check_converter_spec_incomplete,
    },
    // rail-contract-design.md §6.7 converter output vs rail window
    // (window-notes batch) — a regulator's spec.output guarantee is the window
    // it promises to hold on the net its Src row drives. When that net is a
    // declared rail face, the rail's window is the scope's promise for the net
    // (§4.1 priority 1); a guarantee the rail window does not cover can deliver
    // outside the rail's tolerance → Error. Src on a plain driven node is not
    // checked (the rail promise lives on the rail net; the feed side is 6023).
    declare_flat_erc_rule! {
        code = crate::errcodes::POWER_CONVERTER_OUTPUT_RAIL_WINDOW,
        name = "converter-output-rail-window",
        title = "converter output guarantee escapes the declared rail window of the rail net it drives",
        severity = Error,
        domain = Power,
        family = None,
        doc = "§6.7 converter-output vs rail-window cross-check: a def writing spec.output guarantees that window on its Src output net. If that net is a declared rail face the rail's own window is the net's promise (§4.1 priority 1 — the rail wins even over a converter landing on it), so a spec.output the rail window does not cover lets the converter deliver outside the scope's declared tolerance → Error. A Src landing on a plain driven node (buck LX → filter → rail net), or a degenerate rail face (a bare nominal with no ±tol — no allowed spread declared), is not checked — the rail promise lives on the rail net, and the input side of the chain is 6023's gate (rail-contract-design.md §6.7).",
        lock = "tests/power_intent_l1.rs",
        overridable = false,
    ...
    // PI-2 filter-leg load-side decoupling (power-quality-design.md §2.2, ruling
    // 11 decided 2026-09-17); table tail, tracking the FLAT_ERC_ORDER append
    // (§5-5).
    declare_flat_erc_rule! {
        code = crate::errcodes::BRIDGE_LOAD_DECOUPLING_MISSING,
        name = "bridge-load-decoupling",
        title = "a declared filter bridge's load side carries no decoupling capacitor",
        severity = Error,
        domain = Power,
        family = None,
        doc = "PI-2 (power-quality-design.md §2.2): a `@bridge` whose two endpoints are both hot faces of declared rails is a supply filter leg — the ferrite is the series half of a filter, not the filter, and the LC only exists once the load side it protects carries a decoupling element. The load side is read from the declaration, never from the arrow order: `@bridge(A, B)` carries only names and the corpus writes both spellings of one flow, so the protected side is the endpoint whose domain world reads as a quiet/sensitive face (`@class(analog)`, `@noise(quiet)`, `@noise(sensitive)`). Ruling 11 (2026-09-17) cuts the verdict to the existence face — the load-side net is a declared rail's hot member and a capacitor sits on it, or none does; where that capacitor's return leg lands is PI-3's object (decoupling-return-face, 6038), so a mis-landed return is reported once, not twice. Both endpoints are read through the net's effective class and the capacitor candidate from the flat carries (the definition's spec table says capacitive), so the verdict needs no names and no module-depth assumption. Not judged, never guessed: a bridge that is not a supply leg (its endpoints are not both hot faces — a ground-side bridge is one), a bridge with no quiet side, one whose two sides both read quiet (the load side would be a guess), an endpoint whose class does not resolve, and a `@couple` edge (a DC-blocking coupling element is not a filter leg).",
        lock = "tests/power_intent_l1.rs",
        overridable = false,
    ...
    declare_flat_erc_rule! {
        ...
        domain = Power,
        family = None,
        doc = "PI-1 (power-quality-design.md §2.1, ruling 2): a pin that draws from a declared DC pair carries no decoupling capacitor on its hot net. The subject is the load terminal, not the filter — a `psnk` component pin row and a module supply port are judged alike (design §6 R2, the first landing includes ports). The pair is read from the pin's own declared member carry (`InstEntry::pwr_member`, the flat form of the `::DC` row that owns it), never from a name: a pin whose row writes no pair, or whose net resolves no declared face, is not judged (a single-phase AC shape with no return member belongs to axis ④), and neither is a pin of a part marked `nc` at its instance site: an unmounted alternative draws nothing, so the not-fitted read the candidate side applies reaches the subject too (U163, hbl `wm7121(NC)` — its decoupling capacitor is `nc` with it and neither is on the board). The candidate is the flat element class (capacitive, two terminals on two distinct nets). Existence only — ruling 11's partition applies here for the same reason it was drawn for PI-2: whether a capacitor sits on the sink's hot net, while where that capacitor's return leg lands is PI-3's object (decoupling-return-face, 6038). Without that cut one cause would be reported twice. Warning, not Error: no declaration says this rail must be decoupled, so this reports design quality, the same level as PWR-2's return-completeness audit.",
        lock = "tests/power_intent_l1.rs",
        overridable = false,
...
mod tests {
    ...
    const FLAT_ERC_ORDER: [u32; 62] = [
        ...
        POWER_CONVERTER_GATE, // §6.1 regulator input-window gate (window batch)
...
pub static POSTPARSE_RULES: &[PostParseRule] = &[
    ...
    declare_post_parse_rule! {
        ...
        name = "hw-iface-peer-not-mutual",
        title = "interface role peer is not mutual",
        severity = Warning,
        domain = PinDecl,
        host = "hw",
        doc = "Interface role names a peer that does not name it back (interface-connect-rule-design.md §3.3 A).",
        lock = "tests/shard7/iface_connect_rule.rs",
    },
    declare_post_parse_rule! {
        code = crate::errcodes::HW_IFACE_PEER_WIDTH_MISMATCH,
        name = "hw-iface-peer-width-mismatch",
        title = "interface role peer width mismatch",
        severity = Warning,
        domain = PinDecl,
        host = "hw",
