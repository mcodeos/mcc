...
// A stage readout's payload type lives in the library, not here: the MCP server
// is a second binary that reaches this crate's public API and not this module,
// and the two have to publish the same field list (`mcc::stages::payload`).
pub use mcc::stages::payload::StageViewData;
...
pub struct DiagLocation {
    ...
    pub line: u32,
    pub column: u32,
    /// End line. Computed from pos+len when available.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub end_line: Option<u32>,
    /// End column. Computed from pos+len when available.
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// A related location / context note attached to a diagnostic.
#[derive(Debug, Serialize, Deserialize, Clone)]
