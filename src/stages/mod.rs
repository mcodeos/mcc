...
/// first and so far only carried word is `diagnostics`
/// ([`diagview`]), the other five are still v1 reservations, and publishing
/// one of those would be impersonating a projection that does not exist. The
/// lock `tests/shard7/view_vocabulary.rs` reads the canonical words from the
/// CDDL, holds every published word that equals a canonical word to the
/// carried set, and holds this list equal to what the producers actually
/// stamp — a new face adds its name here; no face invents a name anywhere
/// else.
...
pub struct StageView {
    ...
    /// `org-units` / `diagnostics`), which name what they read rather than a
    /// pipeline stage.
    pub view: &'static str,
...
impl StageView {
    ...
    ///
    /// `join`, `trace`, `org-units` and `diagnostics` publish their own `view`
