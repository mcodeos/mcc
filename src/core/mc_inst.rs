        ...
        if let Some(iotype) = IOType::new(&subnode) {
            for each in subnode.iter().skip(1) {
                match each.get_type() {
                    ...
                    MCAST_DECLARE => {
                        // Directly parse MCAST_DECLARE node
                        ...
                        // ★ FIX: Use caller's passed `uri` instead of `McURI::default()`,
                        // otherwise `mcb_get_cmie` can't find companion modules in current file directory,
                        // falls to fallback path generating empty stub, then triggers 1801/1803.
                        let cmie = mcb_get_cmie(&class_ids, uri);

                        // Only process first instance
                        if let Some(inst_node) = inst_nodes.first() {
                            ...
                            let inst_name = inst_ids.to_string();

                            // Collect instance parameters (simplified - no context needed for basic types)
                                                        ...
                                                        MCAST_INT => {
                                                            if let Some(int_val) = McInt::new(&sub)
                                                            {
                                                                instance_params.push(
                                                                    McParamValue::Int(int_val),
                                                                );
                                                            }
                                                        }
                                                        MCAST_OPD_NC => {
                                                            instance_params.push(McParamValue::NC(
                                                                String::from("NC"),
                                                            ));
                                                        }
                            ...
                            let mc_inst = match &cmie {
                                ...
                                // ★ FIX (Issue #1804/#1803):
                                // Use `Mc2Interface::new(inst_ids.clone(), ...)` instead of
                                // `new_with_str(&inst_name, ...)`, so forms like `MIC{P,N}::ADC.DIFF()`
                                // have user-declared bus members preserved in `name: McIds`
                                // for later `validate_interface_member_ref` validation.
                                ...
                                _ => McInstance::Label(inst_name.clone()),
                            };
                            let base = inst_ids.base_name();
                            if base.is_empty() {
                                let port_name = format!("@{}", self.insts.len());
                                self.insts.insert(port_name, (iotype.clone(), mc_inst));
                            } else {
                                self.insts.insert(base.clone(), (iotype.clone(), mc_inst));
                            }
                        }
                    }
