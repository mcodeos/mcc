        ...
        for func in entry.value().funcs.iter() {
            if func.lines.is_empty() && func.insts.is_empty() {
                acc.push(CheckResult {
                    check_name: "extra", severity: CheckSeverity::Warning,
                    uri: Some(uri.clone()), span: None,
                    message: format!("Function '{}' in component '{}' has an empty body.",
                        func.name, entry.key().ident),
                    code: 2602,
                });
            }
        }
        ...
        if comp.pins.pin_ranges.len() > 1 {
            acc.push(CheckResult {
                ...
                message: format!(
                    ...
                    entry.key().ident, comp.pins.pin_ranges.len()
                ),
                code: 2608,
            });
        }
    ...
    for entry in comps.iter() {
        ...
        if !has_params && !has_pins && !has_attrs && !has_funcs {
            acc.push(CheckResult {
                check_name: "extra", severity: CheckSeverity::Warning,
                uri: Some(uri.clone()), span: None,
                message: format!("Component '{}' has no params, pins, attributes, or functions.", name),
                code: 2603,
            });
        }
            ...
            if let Some(def) = d.param_type.default_value() {
                ...
                match &d.param_type.kind {
                    ...
                    McParamTypeKind::BasicInt { .. } | McParamTypeKind::BasicHex { .. } => {
                        if def.starts_with('"') || def.starts_with('\'') {
                            acc.push(CheckResult {
                                ...
                                uri: Some(uri.clone()), span: None,
                                message: format!("Param '{}' is ::INT/HEX but default '{}' is a string.", pname, def),
                                code: 2505,
                            });
                        }
                    }
                    ...
                    McParamTypeKind::BasicString { .. } => {
                        if !def.starts_with('"') && !def.starts_with('\'')
                            ...
                            acc.push(CheckResult {
                                check_name: "extra", severity: CheckSeverity::Warning,
                                uri: Some(uri.clone()), span: None,
                                message: format!("Param '{}' is ::STRING but default '{}' looks numeric.", pname, def),
                                code: 2506,
                            });
                        }
                    }
                    ...
                    McParamTypeKind::UnitValue { unit } | McParamTypeKind::UnitValueDefault { unit, .. } => {
                        if def.chars().all(|c| c.is_ascii_digit() || c == '.' || c == '-') {
...
fn check_instance_class_found(acc: &mut CheckAccumulator) {
    ...
    for e in comps.iter() { known.insert(e.key().ident.to_string()); }
    for e in ifaces.iter() { known.insert(e.key().ident.to_string()); }
    ...
    for entry in modules.iter() {
        ...
        for name in m.insts.iter_instance_names() {
            ...
            if name.starts_with('@') || name.starts_with('[') || name.len() <= 1 { continue; }
            if !known.contains(name) {
                ...
                if name.chars().next().map_or(false, |c| c.is_uppercase()) {
                    acc.push(CheckResult {
                        check_name: "extra", severity: CheckSeverity::Warning,
                        uri: Some(uri.clone()), span: None,
                        message: format!("Instance '{}' references class that is not loaded.", name),
                        code: 2606,
                    });
                }
            }
...
fn check_bus_member_collision(acc: &mut CheckAccumulator) {
    ...
    for entry in modules.iter() {
        ...
        if uri.contains("/unitest/") || uri.contains("/cases") { continue; }
        let m = entry.value();
        let mut bus_members: std::collections::HashMap<String, Vec<String>> = std::collections::HashMap::new();
                        ...
                        for m in &bus.member {
                            if entry.contains(m) {
                                acc.push(CheckResult {
                                    check_name: "extra", severity: CheckSeverity::Warning,
                                    uri: Some(uri.clone()), span: None,
                                    message: format!("Bus '{}' has duplicate member '{}' in module.", bus.name, m),
                                    code: 2609,
                                });
                            } else {
                                entry.push(m.clone());
...
fn check_dry_functions(acc: &mut CheckAccumulator) {
    ...
    for entry in comps.iter() {
        ...
        for func in comp.funcs.iter() {
            ...
            seen.entry(hash).or_default().push(Box::leak(name.into_boxed_str()));
        }
        for (_, names) in &seen {
            if names.len() > 1 {
                acc.push(CheckResult {
...
fn check_naming_convention(acc: &mut CheckAccumulator) {
    ...
    for entry in comps.iter() {
        let name = entry.key().ident.to_string();
        let uri = entry.key().uri.to_string();
        if uri.contains("/unitest/") || uri.contains("/cases") || uri.contains("/lab/") { continue; }
            ...
            if let Some(first) = seg.chars().next() {
                if first.is_lowercase() && seg.chars().any(|c| c.is_uppercase()) {
                    ...
                    acc.push(CheckResult {
                        check_name: "extra", severity: CheckSeverity::Info,
                        uri: Some(uri.clone()), span: None,
                        message: format!("Component '{}' uses mixed case. Convention is UPPER_SNAKE.", name),
                        code: 2607,
                    });
                    break;
                }
