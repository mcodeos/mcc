    ...
    pub(super) fn get_left_points(
        ...
        match phrase {
            ...
            McPhrase::Parallel(phrases) => {
                ...
                // generated directly by stmt.rs::process_member_internal::Parallel
                ...
                if let Some(first) = phrases.first() {
                    match first {
                        ...
                        _ => {
                            ...
                            // stmt.rs Parallel handler pointer-keeping fix hits);
            ...
            McPhrase::Group(ref g) => {
                ...
                for phrase in &g.opds {
                    // ── BUG4 fix (companion to stmt.rs Group handler pointer-keeping fix) ──
                    // prefer get_left_points: FuncCall in branches goes through
                    // resolve_funccall_left_points querying auto_inst_map. paired with stmt.rs
