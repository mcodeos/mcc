...
pub fn dump_layout(graph: &McVecGraph, layouter_name: &str, canvas: (f64, f64)) {
    ...
    mcc_dbg!("viz", "{p} ── BEGIN ─────────────────────────────────");
    mcc_dbg!("viz", "{p} layouter   = {layouter_name}");
    mcc_dbg!(
        "viz",
        "{} canvas     = {:.0} x {:.0}",
        p,
    ...
    mcc_dbg!("viz", "{} boxes      = {}", p, graph.boxes.len());
    ...
    if overlaps > 0 {
        mcc_dbg!("viz", "{p} ⚠ {overlaps} overlapping pair(s) detected");
    }
...
pub fn dump_route(graph: &McVecGraph) {
    ...
    mcc_dbg!("viz", "{p} ── BEGIN ─────────────────────────────────");
    mcc_dbg!("viz", "{} nets       = {}", p, graph.nets.len());
    ...
    if unrouted > 0 {
        mcc_dbg!("viz", "{p} ⚠ {unrouted} net(s) without route");
    }
    mcc_dbg!("viz", "{p} routed={routed} unrouted={unrouted}");
    mcc_dbg!("viz", "{p} ── END ───────────────────────────────────");
}
...
pub fn dump_document(doc: &VizDocument) {
    ...
    let p = "[VIZ-DOC   ]";
    mcc_dbg!("viz", "{p} ── BEGIN ─────────────────────────────────");
    mcc_dbg!("viz", "{} root_bid   = {}", p, doc.root_bid);
    mcc_dbg!("viz", "{} root_name  = {}", p, doc.root_name);
    mcc_dbg!("viz", "{} layers     = {}", p, doc.layer_count());
    mcc_dbg!("viz", "{} total SVG  = {} bytes", p, doc.total_svg_bytes());
