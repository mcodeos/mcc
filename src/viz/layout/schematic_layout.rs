...
use super::entry_points::{
    assign_entry_points_coarse, promote_synthetic_pins, split_shared_pins,
};
...
impl Layouter for SchematicSubLayouter {
    fn layout(&self, graph: &mut McVecGraph) -> (f64, f64) {
        ...
        crate::vlog!(
            ...
            result.hub_name, result.hub_id,
            result.chains.len(), result.orphan_ids.len()
        );
...
fn rebalance_directions(map: &mut DirMap, by_pin: &HashMap<i64, Vec<&SignalChain>>) {
    ...
    if right > left + 2 {
        ...
        let mut candidates: Vec<(i64, usize)> = map.iter()
            .filter(|(_, d)| **d == ChainDir::Right)
            .map(|(&pid, _)| {
                let depth: usize = by_pin.get(&pid)
...
impl SchematicSubLayouter {
    fn place_all(
        &self, graph: &mut McVecGraph, result: &SignalChainResult, dir_map: &DirMap,
    ) {
        // ── Step 1: Place hub IC at center ──
        let hub_id = result.hub_id;
        let (hub_w, hub_h) = graph.boxes.iter()
            .find(|b| b.id == hub_id)
            .map(|b| (b.w, b.h))
        ...
        for &pin_id in &pin_order {
            ...
            for chain in chains {
                self.place_chain(
                    graph, chain, &dir, hub_cx, hub_cy, hub_w, hub_h,
                    *slot, &mut placed,
                );
                *slot += 1;
            }
    ...
    fn place_chain(
        ...
        if chain.loops_to_hub && chain.nodes.len() == 1 {
            ...
            if placed.contains(&node.box_id) { return; }
        ...
        for &box_id in &all_nodes {
            if placed.contains(&box_id) { continue; }
    ...
    fn place_orphans(
        ...
        let mut unplaced: Vec<i64> = graph.boxes.iter()
            .filter(|b| !placed.contains(&b.id))
            .map(|b| b.id)
            .collect();

        if unplaced.is_empty() { return; }
        ...
        for id in &power_labels {
            let (bw, bh) = box_size_of(graph, *id);
...
mod tests {
    ...
    use crate::vector::graph::{
        EndpointRef, EntryPoint, EntrySide, IoSummary, McVecBox, McVecGraph,
        NetKind, VizNet,
    };
    use crate::viz::traits::Layouter;
    ...
    fn mk_ic(id: i64, name: &str, pin_count: usize) -> McVecBox {
        McVecBox::new_v2(
            id, name.into(), "".into(), BoxKind::MultiPin,
            Symbol::Ic, None, None, pin_count, IoSummary::new(),
        )
    }

    fn mk_passive(id: i64, name: &str, sym: Symbol) -> McVecBox {
        McVecBox::new_v2(
            id, name.into(), "".into(), BoxKind::TwoPin,
            sym, Some(name.into()), None, 2, IoSummary::new(),
        )
    }
    ...
    fn mk_label(id: i64, name: &str) -> McVecBox {
        ...
        McVecBox::new_v2(
            id, name.into(), "".into(), BoxKind::PowerLabel,
            Symbol::PowerRail { is_ground: is_gnd }, None, None, 0, IoSummary::new(),
        )
    }

    fn mk_sub(id: i64, name: &str) -> McVecBox {
        McVecBox::new_v2(
            id, name.into(), "".into(), BoxKind::SubModule,
            Symbol::Module, None, None, 0, IoSummary::new(),
        )
    }
    ...
    fn build_moddcdc_graph() -> McVecGraph {
        ...
        ic.entry_points = vec![
            EntryPoint { pin_id: 10, pin_name: "EN".into(), side: EntrySide::Left, offset: 0.25 },
            EntryPoint { pin_id: 11, pin_name: "LX".into(), side: EntrySide::Right, offset: 0.25 },
            EntryPoint { pin_id: 12, pin_name: "GND".into(), side: EntrySide::Bottom, offset: 0.5 },
            EntryPoint { pin_id: 13, pin_name: "Vin".into(), side: EntrySide::Left, offset: 0.5 },
            EntryPoint { pin_id: 14, pin_name: "Vin".into(), side: EntrySide::Left, offset: 0.75 },
            EntryPoint { pin_id: 15, pin_name: "FB".into(), side: EntrySide::Right, offset: 0.5 },
            EntryPoint { pin_id: 16, pin_name: "FB".into(), side: EntrySide::Right, offset: 0.6 },
            EntryPoint { pin_id: 17, pin_name: "FB".into(), side: EntrySide::Right, offset: 0.7 },
        ];
        g.boxes.push(ic);

        // Passives
        g.boxes.push(mk_passive(2001, "C_dcdc_en", Symbol::Capacitor));
        g.boxes.push(mk_passive(2002, "L_dcdc", Symbol::Inductor));
        g.boxes.push(mk_passive(2003, "C_dcdc_vin", Symbol::Capacitor));
        g.boxes.push(mk_passive(2004, "R_fb_high", Symbol::Resistor));
        g.boxes.push(mk_passive(2005, "R_fb_low", Symbol::Resistor));
        g.boxes.push(mk_passive(2006, "C_dcdc_fb", Symbol::Capacitor));
        g.boxes.push(mk_passive(2007, "C_dcdc_vout_10u", Symbol::Capacitor));
        ...
        g.nets.push(mk_net(100, "net_en", vec![
            (1168, 10, "EN"), (2001, 101, "1"),
        ]));
        // EN return: C_dcdc_en -- IC (loop back via different pin on same IC)
        g.nets.push(mk_net(101, "net_en_ret", vec![
            (2001, 102, "2"), (1168, 12, "GND"),
        ]));

        // LX net: IC.LX(pin11) -- L_dcdc
        g.nets.push(mk_net(110, "net_lx", vec![
            (1168, 11, "LX"), (2002, 201, "1"),
        ]));
        // L_dcdc -- C_dcdc_vout_10u
        g.nets.push(mk_net(111, "net_vout", vec![
            (2002, 202, "2"), (2007, 701, "1"),
        ]));

        // GND direct: IC.GND(pin12) -- moddcdc
        g.nets.push(mk_net(120, "net_gnd", vec![
            (1168, 12, "GND"), (9999, 901, "GND"),
        ]));

        // Vin net: IC.Vin(pin13) -- C_dcdc_vin
        g.nets.push(mk_net(130, "net_vin", vec![
            (1168, 13, "Vin"), (2003, 301, "1"),
        ]));
        // Vin return: C_dcdc_vin -- IC (loop back)
        g.nets.push(mk_net(131, "net_vin_ret", vec![
            (2003, 302, "2"), (1168, 12, "GND"),
        ]));

        // Vin direct: IC.Vin(pin14) -- moddcdc
        g.nets.push(mk_net(140, "net_vin_direct", vec![
            (1168, 14, "Vin"), (9999, 902, "Vin"),
        ]));

        // FB nets: IC.FB(pin15) -- R_fb_high
        g.nets.push(mk_net(150, "net_fb_h", vec![
            (1168, 15, "FB"), (2004, 401, "1"),
        ]));
        // R_fb_high -- C_dcdc_vout_10u (terminus)
        g.nets.push(mk_net(151, "net_fb_h_out", vec![
            (2004, 402, "2"), (2007, 702, "2"),
        ]));

        // IC.FB(pin16) -- R_fb_low
        g.nets.push(mk_net(160, "net_fb_l", vec![
            (1168, 16, "FB"), (2005, 501, "1"),
        ]));
        // R_fb_low -- moddcdc
        g.nets.push(mk_net(161, "net_fb_l_out", vec![
            (2005, 502, "2"), (9999, 903, "GND"),
        ]));

        // IC.FB(pin17) -- C_dcdc_fb
        g.nets.push(mk_net(170, "net_fb_c", vec![
            (1168, 17, "FB"), (2006, 601, "1"),
        ]));
        // C_dcdc_fb -- moddcdc
        g.nets.push(mk_net(171, "net_fb_c_out", vec![
            (2006, 602, "2"), (9999, 904, "FB_GND"),
        ]));
    ...
    fn moddcdc_hub_centered_passives_radiate() {
        ...
        assert!(
            ...
            l_dcdc.x, hub.x + hub.w
        );

        // C_dcdc_vout_10u should be further right than L_dcdc (it's downstream)
        let c_vout = g.boxes.iter().find(|b| b.name == "C_dcdc_vout_10u").unwrap();
        assert!(
            c_vout.x >= l_dcdc.x,
