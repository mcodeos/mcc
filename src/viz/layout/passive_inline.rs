...
/// Synthesized second-pin id base for passives whose two pins share one id
/// (distinct from `SPLIT_PIN_BASE` 4e9 and below `FLAG_ID_BASE` 9e9).
    ...
    for pid in passive_ids {
        ...
        if touching.len() != 2 {
            ...
            eprintln!(
                ...
                touching
                    ...
                    .map(|&i| (
                        graph.nets[i].nid,
                        graph.nets[i].name.as_str(),
                        graph.nets[i].endpoints.len()
                    ))
                    .collect::<Vec<_>>()
            );
        ...
        for &ni in &touching {
            ...
            match (p_pin, others.as_slice()) {
                ...
                (Some(pp), [o]) if !passive_set.contains(&o.box_id) => {
                ...
                _ => {
                    let box_name = graph
                        ...
                        .map(|b| b.name.as_str())
                        .unwrap_or("?");
                    let other_info: Vec<(i64, i64)> =
                        others.iter().map(|o| (o.box_id, o.pin_id)).collect();
                    eprintln!(
        ...
        if has_sentinel {
            ...
            let passive_center = (
                passive_box.x + passive_box.w / 2.0,
                passive_box.y + passive_box.h / 2.0,
            );
            ...
            let (ax, ay) = if real_idx == 0 {
                real_exit
            } else {
                sentinel_pt
            };
            let sa = if real_idx == 0 {
                real_side
            } else {
                sentinel_side
            };
            let (bx2, by2) = if real_idx == 0 {
                sentinel_pt
            } else {
                real_exit
            };
            let sb = if real_idx == 0 {
                sentinel_side2
            } else {
                real_side2
            };
...
// favor of the non-destructive `place_series_passives`. Rail-connected passives are now
// placed by the same pass; `straighten_rail_passives` is dead code and deleted.
//
// Runs **after** layout, **before** routing. Root + sub layers.
...
pub fn stand_grounded_passives(graph: &mut McVecGraph) -> usize {
    ...
    for bi in 0..graph.boxes.len() {
        ...
        let already_rung = graph.boxes[bi].h > graph.boxes[bi].w
            && matches!(
                graph.boxes[bi].entry_points[0].side,
                EntrySide::Top | EntrySide::Bottom
            )
            && matches!(
                graph.boxes[bi].entry_points[1].side,
                EntrySide::Top | EntrySide::Bottom
            );
...
/// current entry order.
fn orientation_indices(dec_ground: &HashSet<(i64, i64)>, b: &McVecBox) -> (usize, usize) {
    let g0 = dec_ground.contains(&(b.id, b.entry_points[0].pin_id));
    let g1 = dec_ground.contains(&(b.id, b.entry_points[1].pin_id));
    if g0 {
        (1, 0)
