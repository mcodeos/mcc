...
//! constant, a name, and a description. Generated from
...
static ALL_CODES: &[ErrorCodeInfo] = &[
    // ---- section ----
    entry!(DUP_INTERFACE, "An interface with the same name already exists in this file."),
    entry!(DUP_COMPONENT, "A component with the same name already exists in this file."),
    entry!(DUP_ENUM, "An enum with the same name already exists in this file."),
    entry!(DUP_MODULE, "A module with the same name already exists in this file."),
    entry!(DUP_DEFINE, "A define with the same name already exists in this file."),
    // ---- section ----
    entry!(DEF_ALREADY_EXISTS, "Definition already exists."),
    entry!(CMIE_LOAD_REJECTED, "The declaration is a define or an unexpected type and cannot be loaded as a CMIE."),
    entry!(INST_MISSING_SUBNODE, "Missing subnode in an instance declaration."),
    entry!(PINS_MISSING_SUBNODE, "Missing subnode in a pins declaration."),
    entry!(ENUM_MISSING_SUBNODES, "Enum definition is missing its subnodes."),
    entry!(ENUM_MISSING_NAME, "Enum definition is missing its name."),
    entry!(ENUM_MISSING_NAME_IDS, "Enum definition is missing its name ids."),
    entry!(ENUM_MISSING_VALUES, "Enum definition is missing its values."),
    entry!(MALFORMED_IOTYPE, "Malformed IO type node in a pin/port declaration."),
    // ---- section ----
    entry!(USE_PATH_INVALID, "Invalid path in a use statement."),
    entry!(USE_URI_PREFIX_INVALID, "Unrecognized URI prefix — expected $, /, ./, or ../."),
    entry!(USE_TARGET_NOT_FOUND, "The use target file was not found."),
    entry!(USE_SELF_IMPORT, "File imports itself via a use statement."),
    entry!(USE_ALIAS_COLLISION, "A use alias collides with an existing name."),
    entry!(USE_VERSIONED_TARGET_NOT_FOUND, "The versioned use target file was not found."),
    entry!(USE_IMPORT_SYMBOL_NOT_FOUND, "A symbol listed in use import(...) was not found in the target file."),
    entry!(USE_REEXPORT_SYMBOL_NOT_FOUND, "A symbol in pub use import(...) was not found and cannot be re-exported."),
    entry!(USE_MIXED_PATH_SEPARATORS, "A use path mixes '.' and '/' separators."),
    // ---- section ----
    entry!(USE_DEP_NOT_DECLARED, "Use of an undeclared dependency — add it to project.toml [dependencies] or load via --lib."),
    entry!(USE_SYMBOL_CONFLICT, "An imported symbol conflicts with an existing name."),
    entry!(USE_IMPORTED_NOT_FOUND, "The imported symbol was not found in the target file."),
    // ---- section ----
    entry!(PARSER_SYNTAX_ERROR, "Generic syntax error."),
    entry!(PARSER_TOP_INVALID, "Invalid top-level declaration."),
    entry!(PARSER_CLAUSE_INVALID, "Invalid clause in a body."),
    entry!(PARSER_PIN_INVALID, "Invalid pin declaration."),
    entry!(PARSER_PIN_ID_NOT_CONST, "Pin ID must be a constant integer, not an expression."),
    entry!(PARSER_PIN_NAME_NOT_CONST, "Pin name must be a constant identifier, not an expression."),
    entry!(PARSER_NET_NOT_PORT, "Net endpoint must be a port/label, not a literal."),
    entry!(PARSER_NET_INVALID, "Invalid net/connection expression."),
    entry!(PARSER_CONDS_INVALID, "Invalid if/else condition block."),
    entry!(PARSER_ROLE_INVALID, "Invalid role block."),
    entry!(PARSER_FUNC_INVALID, "Invalid function definition."),
    entry!(PARSER_PINS_INVALID, "Invalid pins declaration."),
    entry!(PARSER_USE_INVALID, "Invalid import statement."),
    entry!(PARSER_CONDBLOCK_INVALID, "Invalid condition body."),
    entry!(PARSER_DECLAREB_INVALID, "Invalid instance declaration (:: syntax)."),
    entry!(PARSER_BODY_INVALID, "Invalid body."),
    entry!(PARSER_JUDGE_INVALID, "Invalid condition expression."),
    entry!(PARSER_PARD_INVALID, "Invalid parameter declaration."),
    entry!(PARSER_URI_INVALID, "Invalid import path."),
    entry!(PARSER_PHRASES_INVALID, "Invalid expression list."),
    entry!(PARSER_OPDS_INVALID, "Invalid operand list."),
    entry!(PARSER_PARAMS_INVALID, "Invalid parameter list."),
    entry!(PARSER_PARDS_INVALID, "Invalid parameter declaration list."),
    entry!(PARSER_ATTR_VALUES_INVALID, "Invalid attribute value list."),
    entry!(PARSER_ATTR_LINES_INVALID, "Invalid attribute line list."),
    entry!(PARSER_PINS_NAMES_INVALID, "Invalid pin name list."),
    entry!(PARSER_INSTS_INVALID, "Invalid instance list."),
    entry!(PARSER_CONDS_ELIFS_INVALID, "Invalid else-if chain."),
    entry!(PARSER_IDSS_INVALID, "Invalid identifier list."),
    entry!(PARSER_LEVELS_INVALID, "Invalid path in import."),
    entry!(PARSER_PHRASE_INVALID, "Invalid expression."),
    entry!(PARSER_SINGLE_OR, "Single '|' used as a binary operator outside a pin context."),
    entry!(PARSER_PLUSMINUS, "'±' used as a binary operator outside a tolerance context."),
    entry!(PARSER_TRANSPOSE_ON_LITERAL, "Transpose (') on a literal has no effect."),
    entry!(PARSER_CARET_ON_LITERAL, "Caret (^) on a literal has no effect."),
    entry!(PARSER_EMPTY_BODY, "Empty body — no clauses defined."),
    entry!(PARSER_EMPTY_PINS, "Empty pins declaration."),
    entry!(AST_NODE_EMPTY, "AST node is null/empty where a value was expected."),
    entry!(AST_UTF8_ERROR, "AST node contains invalid UTF-8 data."),
    entry!(AST_TYPE_MISMATCH, "AST node has an unexpected type."),
    // ---- section ----
    entry!(NAME_IDS_NO_NODES, "IDS has no nodes."),
    entry!(NAME_MISSING_SUBNODE, "Missing subnode in a name reference."),
    entry!(NAME_DECLARE_PARSE_FAILED, "Failed to parse a DECLARE node."),
    entry!(NAME_SQUARE_VECTOR_MISSING_SUBNODE, "Missing subnode for a square vector."),
    entry!(NAME_RANGE_SIDE_FAILED, "Failed to process a side of a range."),
    entry!(NAME_DEF_NOT_FOUND_LABEL_FALLBACK, "Definition not found; falling back to a label."),
    entry!(NAME_ID_EXTRACT_FAILED, "Failed to extract ID/IDA data from a node."),
    entry!(NOT_SUPPORTED_YET, "This syntax is parsed but not yet supported by the semantic layer; the declaration is ignored."),
    // ---- section ----
    entry!(PIN_ID_NAME_MISMATCH, "Pin ID and pin name do not match."),
    entry!(PIN_ID_COUNT_ERROR, "Pin id count error."),
    entry!(PINS_PLUS_WITHOUT_BASE, "pins += is used without a prior pins = definition."),
    entry!(PIN_NAME_TYPE_UNSUPPORTED, "Pin name has an unsupported type."),
    entry!(PIN_NAME_COUNT_ERROR, "Pin/port name count error."),
    entry!(PORT_NAME_TYPE_UNSUPPORTED, "Port name has an unsupported type."),
    entry!(PORT_NAME_COUNT_ERROR, "Port name count error."),
    entry!(PIN_EXPR_TYPE_MISMATCH, "Pin expression node has an unexpected type."),
    entry!(ATTR_TYPE_MISMATCH, "Attribute node type mismatch."),
    entry!(ATTR_TYPE_NOT_SUPPORTED, "Attribute type is not supported."),
    entry!(KVS_VALUE_TYPE_INVALID, "Invalid value type in a KVS node."),
    entry!(UVAL_VALUE_TYPE_INVALID, "Invalid unit value type."),
    entry!(UVAL_DATA_NODE_INVALID, "Invalid unit value data node."),
    entry!(UVAL_UNIT_INVALID, "Invalid unit."),
    entry!(UVAL_VALUE_INVALID, "Invalid unit value."),
    entry!(UVAL_UNIT_UNSUPPORTED, "The unit is not supported."),
    entry!(UVAL_MISSING_DATA_NODE, "Missing unit value data node."),
    entry!(UVAL_FORMAT_INVALID, "Invalid unit value or float format."),
    entry!(UVAL_UNIT_VARIANT_INVALID, "Invalid unit variant (angle, charge, magnetic flux, slew rate, ...)."),
    // ---- section ----
    entry!(MODULE_MISSING_SUBNODE, "Missing subnode in a module body clause."),
    entry!(MODULE_PINS_UNSUPPORTED, "Module does not support PINS directly; use in/out/io declarations."),
    entry!(MODULE_ROLE_UNSUPPORTED, "Module does not support role definition."),
    entry!(MODULE_PARAM_TYPE_UNEXPECTED, "Unexpected type in a module parameter."),
    entry!(MODULE_METHOD_NOT_FOUND, "Function was not found in the class."),
    entry!(UNEXPECTED_CLAUSE_TYPE, "Unexpected clause type in a module body."),
    // ---- section ----
    entry!(FUNC_EMPTY_NET, "Empty net in a function or module body."),
    entry!(PARAM_DECLARE_INVALID, "Invalid parameter declaration node."),
    entry!(PARAM_NAME_INVALID, "Invalid parameter name."),
    entry!(PARAM_SET_INVALID, "Invalid parameter set."),
    entry!(PARAM_UVAL_INVALID, "Invalid parameter unit value."),
    entry!(PARAM_CLASS_EXPECTED, "Expected a class in the declaration unit value."),
    entry!(PARAM_INSTANCE_EXPECTED, "Expected an instance in the declaration unit value."),
    entry!(PARAM_NAME_EXTRACT_FAILED, "Failed to extract the parameter name."),
    entry!(FUNC_CALL_MISSING_NAME, "Missing function name in a function call."),
    entry!(CONN_LINE_PARSE_FAILED, "A connection line failed to parse."),
    entry!(FUNC_BODY_INVALID, "Invalid function body node."),
    entry!(FUNC_LINE_DROPPED, "A connection line was dropped because McPhrase::new returned None."),
    entry!(FCALL_PARSE_FAILED, "Function call parse failure."),
    // ---- section ----
    entry!(INST_EXPR_PARSE_FAILED, "Failed to parse an instance in an expression context."),
    entry!(CURLY_MN_WRONG_BASE, "Curly-member construction requires a component or module base."),
    entry!(INST_CLASS_NODE_MISSING, "No class node found in the instance declaration."),
    entry!(INST_NODE_MISSING, "No instance node found."),
    entry!(INST_CLASS_ID_MISSING, "Missing class id node."),
    entry!(INST_CLASS_IDS_PARSE_FAILED, "Failed to parse class ids."),
    entry!(INST_CLASS_UNRESOLVED, "Unresolved class — the library may not be loaded."),
    entry!(FUNC_RETURN_MALFORMED, "Malformed return statement."),
    entry!(FUNC_RETURN_EXPR_INVALID, "Invalid return expression — expected this or a label/bus."),
    entry!(FUNC_MULTIPLE_RETURNS, "A function may have at most one return statement."),
    entry!(IFACE_MEMBER_NOT_FOUND, "Interface member not found in the component."),
    entry!(IFACE_CURLY_MEMBER_INVALID, "Cannot access interface members using curly-bracket syntax."),
    entry!(IFACE_COMPONENT_NOT_FOUND, "Component not found for the interface reference."),
    entry!(IFACE_BUS_NOT_FOUND, "Interface not found for a bus reference."),
    entry!(MODULE_PORT_NOT_FOUND, "Port(s) not found in the module."),
    entry!(BUS_NAME_ALREADY_INSTANCE, "Name is already an instance; cannot create a bus with these members."),
    entry!(IFACE_PIN_NOT_FOUND, "Pin(s) not found in the interface."),
    entry!(IFACE_MEMBER_LOOKUP_FAILED, "Interface member lookup failed."),
    entry!(COMPONENT_PIN_NOT_FOUND, "Pin(s) not found in the component or interface."),
    // ---- section ----
    entry!(CONN_TRANSPOSE_SIZE_MISMATCH, "Transposed connection size mismatch."),
    entry!(CONN_LEFT_ARROW_SHAPE_MISMATCH, "Shape mismatch in a <- connection."),
    entry!(CONN_CANNOT_TRANSPOSE, "The transpose operator is not allowed at this position."),
    entry!(CONN_PARALLEL_INVALID, "An instance with 3+ pins cannot directly participate in a '+' operation."),
    entry!(CONN_PARALLEL_SHAPE_MISMATCH, "Shape mismatch in a parallel connection."),
    entry!(CONN_SERIES_INVALID, "An instance with 3+ pins cannot directly participate in a '-' operation."),
    entry!(CONN_SERIES_SHAPE_MISMATCH, "Shape mismatch in a -> connection."),
    entry!(CONN_OPERATOR_UNSUPPORTED, "The operator is not supported in connection lines; use '+' for parallel, '-' / '->' for series."),
    entry!(PHRASE_AST_TYPE_UNEXPECTED, "Unexpected AST node type in a phrase."),
    entry!(CONN_NO_PORTS_COMPONENT, "No ports found in the component."),
    entry!(CONN_NO_PORTS_MODULE, "No ports found in the module."),
    entry!(CONN_NO_PORTS_INTERFACE, "No ports found in the interface."),
    entry!(CONN_DOT_SERIES, "Dot operator does not apply to a Series."),
    entry!(CONN_DOT_NODE, "Dot operator does not apply to a Node."),
    entry!(CONN_DOT_TRANSPOSED, "Dot operator does not apply to a Transposed."),
    entry!(CONN_DOT_LEAD, "Dot operator does not apply to a Lead."),
    entry!(CONN_DOT_GROUP, "Dot operator does not apply to a Group."),
    entry!(CONN_DOT_ENDPOINT, "Dot operator does not apply to an Endpoint."),
    entry!(CONN_DOT_MEMBER, "Dot operator already applied to a Member."),
    entry!(CLOSURE_NO_OUTPUT_IFACE, "Closure has no output interface to access."),
    entry!(FUNCCALL_NO_RETURN_IFACE, "FuncCall has no return interface to access."),
    entry!(PHRASE_IFACE_MEMBER_NOT_FOUND, "Member not found in the interface."),
    entry!(CURLY_LEFT_EMPTY, "Curly-member construction: left member list is empty."),
    entry!(CURLY_RIGHT_EMPTY, "Curly-member construction: right member list is empty."),
    entry!(CURLY_NOT_BUS, "Cannot convert the curly-member result to a bus."),
    entry!(GROUP_BRANCH_COUNT_MISMATCH, "Groups with different branch counts cannot connect."),
    entry!(CONN_AMBIGUOUS_PRECEDENCE, "Expression mixes + (parallel) with - or -> (series) operators without parentheses, spanning more than two components."),
    // ---- section ----
    entry!(NET_MERGED_SHORT, "Multiple points resolve to the same node — possible short circuit (E2003)."),
    entry!(NET_BUS_ORDER_MISMATCH, "Bus member order mismatch after sorting (E2005)."),
    entry!(SORT_HAZARD, "Bus pin numbers are non-monotonic; member→pin mapping may be wrong after sorting."),
    entry!(FLOATING_PLACEHOLDER, "A '_' placeholder could not be bound to any pin."),
    entry!(GHOST_PORT, "A net endpoint is not mapped to any box — possible unexposed module boundary port."),
    entry!(PULLUP_DEGENERATE, "Pullup/pulldown degenerated into a signal-signal bridge."),
    entry!(NET_DROPPED_STATEMENT, "A single-element square bracket expands to an unknown instance; the statement may produce no nets or constraints."),
    entry!(LAYOUT_MISSING_SUBNODE, "Layout attribute is missing a required subnode."),
    entry!(LAYOUT_TYPE_MISMATCH, "Layout attribute node type mismatch."),
    entry!(LAYOUT_SET_MISSING_SUBNODE, "Layout set is missing a required subnode."),
    entry!(LAYOUT_VALUES_TYPE_MISMATCH, "Layout values node type mismatch."),
    entry!(LAYOUT_NAME_MISSING_SUBNODE, "Layout name is missing a required subnode."),
    entry!(LAYOUT_EDGE_MISSING_SUBNODE, "Layout edge is missing a subnode."),
    entry!(LAYOUT_EDGE_TYPE_MISMATCH, "Layout edge node type mismatch."),
    entry!(LAYOUT_EDGE_NAME_MISSING_SUBNODE, "Layout edge name is missing a subnode."),
    entry!(LAYOUT_VALUE_MISSING_SUBNODE, "Layout value is missing a subnode."),
    entry!(LAYOUT_VALUE_TYPE_MISMATCH, "Layout value node type mismatch."),
    entry!(LAYOUT_SET_SUBNODE_MISSING, "Layout set is missing a subnode."),
    entry!(LAYOUT_EXTRA_NODES, "Malformed layout: unexpected extra nodes."),
    entry!(LAYOUT_VALUES_MISSING_SUBNODE, "Layout values are missing a subnode."),
    entry!(LAYOUT_CONST_MISSING_INT, "CONST node is missing its INT subnode."),
    entry!(LAYOUT_PIN_NUMBER_PARSE, "Parse error in a layout pin number."),
    entry!(LAYOUT_EDGE_NAME_ID_MISSING_SUBNODE, "Layout edge name id is missing a subnode."),
    entry!(LAYOUT_EDGE_INVALID, "Invalid layout edge."),
    entry!(LAYOUT_EDGE_NAME_NOT_ID, "Malformed layout: edge name is not an ID."),
    // ---- section ----
    entry!(NET_MULTI_DRIVE, "Net has multiple drivers — possible short circuit."),
    entry!(IFACE_PINS_NOT_ALL_BOUND, "Interface requires more pins than are bound to physical pins."),
    entry!(NET_NO_DRIVER, "Net has inputs but no output/power driver."),
    entry!(IFACE_ROLE_NOT_FOUND, "Interface role referenced by a param does not exist in the interface."),
    entry!(NET_VOLTAGE_MISMATCH, "Power nets with different voltages are shorted together."),
    entry!(IFACE_NOT_LOADED, "Interface referenced by a param is not loaded."),
    entry!(IFACE_DEPRECATED_CMIE, "Deprecated interface/component/param used."),
    entry!(NET_INPUT_UNCONNECTED, "An input port is not connected to any net."),
    entry!(NET_NC_CONNECTED, "An NC port is connected to a net."),
    entry!(NET_OUTPUT_UNDRIVEN, "An output drives nothing."),
    entry!(NET_BACKFEED_RISK, "Net has both an output and a power supply — backfeed risk."),
    entry!(NET_INSTANCE_UNCONNECTED, "Instance has no pins connected to any net."),
    entry!(NET_OUTPUTS_NO_INPUT, "Net has outputs and power but no input."),
    entry!(NET_MODULE_PORT_UNCONNECTED, "Module port is not connected to any net."),
    entry!(NET_DANGLING_ENDPOINT, "Net has only one endpoint — possible dangling connection."),
    entry!(NET_PARTIAL_CONNECTION, "Only some of the instance pins are connected."),
    entry!(NET_BIDIR_UNCONNECTED, "A bidirectional port is not connected to any net."),
    entry!(NET_POWER_NET_COUNT, "Design has many power nets; review for consolidation."),
    // ---- section ----
    entry!(INST_ARG_NO_FORMAL_PORT, "Instance argument has no formal port to bind."),
    entry!(INST_METHOD_FALLBACK, "Instance method could not be resolved; passed through instead."),
    entry!(INST_IFACE_INSTANTIATE_FAILED, "Interface instantiation failed."),
    entry!(INST_SUBMODULE_INSTANTIATE_FAILED, "Sub-module instantiation failed."),
    entry!(INST_LINE_SKIP_FAILED_CLASS, "Line references a component class whose instantiation failed; the whole line is skipped."),
    entry!(INST_LINE_PARSE_FAILED, "A connection line failed to expand."),
    entry!(INST_BUILTIN_TWOPIN_EXPAND_FAILED, "Expanded builtin two-pin pair failed."),
    entry!(INST_MEMBER_PROCESS_FAILED, "A member of a connection line failed to process."),
    entry!(INST_ADJACENT_CONNECT_FAILED, "Connection between adjacent members of a series failed."),
    entry!(INST_SHUNT_PROCESS_FAILED, "A `.Cap(_)` shunt member failed to process."),
    entry!(INST_FUNC_BODY_LINE_FAILED, "A module-level function body line failed."),
    entry!(INST_LANE_FUNCCALL_FAILED, "Failed to instantiate a FuncCall during lane-by-lane wiring."),
    entry!(INST_LANE_TRANSPOSED_FAILED, "Failed to instantiate a Transposed member during lane-by-lane wiring."),
    entry!(CONN_SHAPE_MISMATCH_TRUNCATED, "Connection shape mismatch; truncated to the smaller side."),
    entry!(CONN_PARALLEL_DIM_MISMATCH, "Parallel '+' operand dimension mismatch; operand merged into the anchor's left net."),
    entry!(CONN_GROUP_SHAPE_MISMATCH, "Group connection shape mismatch; truncated by branch count."),
    entry!(INST_INPUT_PIN_COUNT_MISMATCH, "Component input pin count mismatch in a function call."),
    entry!(INST_OUTPUT_PIN_COUNT_MISMATCH, "Component output pin count mismatch in a function call."),
    entry!(INST_INLINE_MODULE_FAILED, "Inline module instantiation failed."),
    entry!(INST_INPUT_PORT_COUNT_MISMATCH, "Module input port count mismatch in a function call."),
    entry!(INST_OUTPUT_PORT_COUNT_MISMATCH, "Module output port count mismatch in a function call."),
    entry!(INST_POWER_PORT_UNBOUND, "Sub-module DC power port is never connected (missing power argument?)."),
    entry!(INST_CTOR_BODY_LINE_FAILED, "A constructor function body line failed."),
    entry!(INST_CTOR_PARAM_BIND_FAILED, "Constructor parameter binding failed."),
    // ---- section ----
    entry!(DUP_CMIE_CROSS_FILE, "Same name defined in another file (cross-file duplicate)."),
    entry!(DUP_WITHIN, "Duplicate definition within the same declaration."),
    entry!(DUP_ENUM_VALUE, "Enum value appears more than once in the enum."),
    // ---- section ----
    entry!(NAME_COMPONENT_LOWERCASE, "Component name starts with lowercase; convention is UPPER_SNAKE."),
    entry!(NAME_PORT_SHADOWS_CMIE, "Port name shadows a library CMIE name."),
    entry!(NAME_PIN_MIXED_CONVENTION, "Pins use mixed naming conventions."),
    entry!(NAME_INSTANCE_SINGLE_CHAR, "Instance name is a single character."),
    entry!(NAME_PIN_NUMERIC, "Pin name is purely numeric."),
    entry!(NAME_PORT_INST_SHADOWS_CMIE, "Port/instance name shadows a library CMIE name."),
    entry!(NAME_PARAM_SHADOWS_CMIE, "Parameter name shadows a library CMIE name."),
    // ---- section ----
    entry!(SPEC_KEY_UNDECLARED_PARAM, "Spec key references a parameter that is not declared."),
    entry!(REF_INTEGRITY, "Reference integrity violation."),
    entry!(FUNC_PARAMS_NO_BODY, "Function has parameters but no body (empty implementation)."),
    entry!(EXPR_PINS_X_UNDEFINED, "pins.X references an undefined pin name."),
    // ---- section ----
    entry!(INST_DECLARED_MULTIPLE, "Instance is declared more than once in the module."),
    entry!(PORT_DUPLICATE_NAME, "Duplicate port name in the module — ambiguous."),
    entry!(NOT_AN_INTERFACE, "The class is a component/module/enum, not an interface."),
    entry!(NAME_PARAM_AND_INSTANCE, "Name is both a value parameter and an instance."),
    entry!(PIN_UNCONNECTED, "Pin is not connected to any net."),
    entry!(PIN_CONFLICTING_OPTIONS, "Pin uses conflicting option names."),
    entry!(FUNC_RETURN_OUTSIDE_FUNCTION, "Return statement used outside a function."),
    entry!(FUNC_RETURN_LITERAL_INVALID, "Return statement specifies a literal instead of an endpoint."),
    entry!(INST_EMPTY_TABLE, "Empty instance table in a [] :: TYPE declaration."),
    entry!(INST_THIS_TYPE, "this :: TYPE declaration is not allowed."),
    entry!(FUNC_ROLE_AS_ARG, "Role used as a function-call argument."),
    entry!(MODULE_PORT_UNUSED, "Module port is declared but never connected."),
    entry!(COND_SINGLE_BINARY, "Condition compares against a single binary value."),
    // ---- section ----
    entry!(ENUM_SINGLE_VALUE, "Enum has only one value."),
    entry!(PARAM_INT_DEFAULT_STRING, "Integer param has a string default."),
    entry!(PARAM_STRING_DEFAULT_NUMERIC, "String param has a numeric-looking default."),
    entry!(PARAM_UV_DEFAULT_NO_UNIT, "Unit-value param default has no unit suffix (e.g. '5V')."),
    entry!(PARAM_FLOAT_DEFAULT_INVALID, "Param has an invalid float default."),
    entry!(PARAM_NEGATIVE_DEFAULT, "Integer param default is negative."),
    // ---- section ----
    entry!(PARAM_RESERVED_KEYWORD, "Parameter uses a reserved keyword."),
    entry!(FUNC_EMPTY_BODY, "Function has an empty body."),
    entry!(COMPONENT_EMPTY, "Component has no params, pins, attributes, or functions."),
    entry!(COMPONENT_NO_PINS, "Component has no pin definitions."),
    entry!(INTERFACE_EMPTY, "Interface has no pins or roles."),
    entry!(INST_CLASS_NOT_LOADED, "Instance references a class that is not loaded."),
    entry!(COMPONENT_MIXED_CASE, "Component name uses mixed case; convention is UPPER_SNAKE."),
    entry!(BUS_DUPLICATE_MEMBER, "Bus has a duplicate member."),
    entry!(COMPONENT_DUPLICATE_FUNC_BODY, "Component has functions with the same body."),
    entry!(DEFINE_NO_ATTRS, "Define has no attributes."),
    entry!(DEFINE_NON_ATTR_CLAUSE, "Define contains a non-attribute clause."),
    entry!(IFACE_PIN_COUNT_MISMATCH, "Interface expects more pins than are bound."),
    entry!(FUNC_SHARES_NAME_WITH_PORT, "Function shares its name with a port/param."),
    entry!(NET_BOTH_OUTPUTS, "Net connects two outputs."),
    entry!(FUNC_INLINE_BODY_LITERAL_ARG, "Inline function body literal used as a call argument."),
    entry!(FUNC_PARAMS_UNUSED, "Function declares parameters it never uses."),
    entry!(SPEC_KEY_DUPLICATE, "Spec key appears more than once."),
    // ---- section ----
    entry!(DEF_AMBIGUOUS_NAME, "Same name used for different definition kinds."),
    entry!(DEF_REF_NOT_LOADED, "Definition references a class that is not loaded."),
    entry!(COMPONENT_INT_SUFFIX, "Component has an unconventional '.int' suffix."),
    entry!(ENUM_INT_SUFFIX, "Enum has an unconventional '.int' suffix."),
    // ---- section ----
    entry!(ATTR_RESERVED_KEYWORD, "Attribute uses a reserved keyword."),
    entry!(INST_ARG_COUNT_MISMATCH, "Instance passes more/fewer args than the class declares."),
    entry!(ROLE_EMPTY_BODY, "Role has an empty body."),
    entry!(ROLE_NAME_SHADOWS, "Role shares its name with a parameter or pin/port."),
    entry!(ATTR_NESTING_TOO_DEEP, "Attribute nesting depth exceeds 16."),
    entry!(ATTR_PIN_GROUP_UNDEFINED, "Attribute references an undefined pin group, or role used outside a component."),
    entry!(PINS_PLUS_AND_PINS_CONFLICT, "Component mixes pins = and pins.X = attributes, or uses a non-constant default."),
    // ---- section ----
    entry!(ENUM_DUPLICATE_VALUE, "Enum has a duplicate value."),
    entry!(ENUM_MEMBER_DOT, "Enum member contains a dot."),
    entry!(ENUM_MEMBER_LEADING_DIGIT, "Enum member starts with a digit."),
    entry!(ENUM_MEMBER_RESERVED, "Enum member is a reserved keyword."),
    entry!(ATTR_INFINITE_FLOAT, "Attribute has an infinite float value."),
    entry!(ATTR_LARGE_INT, "Attribute has a suspiciously large integer value."),
    entry!(RANGE_REVERSED, "Range appears reversed; did you mean the opposite order?"),
    entry!(RANGE_SINGLE_ELEMENT, "Range expands to a single element."),
    entry!(IDX_MULTIPLE_SLICE_SPEC, "IDX key has multiple slice specifications."),
    entry!(EXPR_THIS_TOP_LEVEL, "'this' used in a top-level net line; it is only valid inside instance/function contexts."),
    entry!(EXPR_PLACEHOLDER_ONLY, "Net connects only to '_' placeholder; the connection has no effect."),
    entry!(ATTR_SELF_REFERENTIAL, "Attribute value equals its own key; likely a copy-paste mistake."),
    // ---- section ----
    entry!(COND_EMPTY_BODY, "Conditional block has an empty body."),
    entry!(COND_IF_WITHOUT_ELSE, "if without a matching else."),
    entry!(PIN_NC_COMPONENT_LEVEL, "NC pin used at component level."),
    entry!(POWER_PIN_NO_VOLTAGE, "Power pin has no voltage attribute."),
    entry!(PIN_IO_MIX_IN_OUT, "Pin mixes In and Out IO types."),
    entry!(PIN_IO_MIX_OUTPUT_POWER, "Pin mixes Output and Power IO types."),
    entry!(PIN_IO_MIX_ANALOG_POWER, "Pin mixes Analog and Power IO types."),
    entry!(PARAM_PIN_NAME_SHADOW, "Parameter shares its name with a pin."),
    entry!(MODULE_STUB, "Module is a stub."),
    // ---- section ----
    entry!(HW_POWER_PINS_EXCESS, "Too many power pins."),
    entry!(HW_PIN_NUMBER_GAP, "Pin numbers have gaps."),
    entry!(HW_PIN_COUNT_HIGH, "Pin count is unusually high."),
    entry!(HW_ZERO_PINS_WITH_PARAMS, "Component has zero pins but parameter attributes."),
    entry!(HW_NC_PINS_CONTIGUOUS, "Consecutive NC pins."),
    entry!(HW_IFACE_ROLE_UNBOUND, "Interface role is never bound."),
    entry!(HW_ALL_SAME_IO_TYPE, "All pins have the same IO type."),
    entry!(HW_MISSING_NAME_ATTR, "Missing 'name' attribute."),
    entry!(HW_NAME_WITHOUT_DESC, "Has a name but no description."),
    entry!(HW_FUNC_PARAM_SHADOWS_PIN, "Function parameter shadows a pin name."),
    entry!(HW_IFACE_NEVER_BOUND, "Interface is defined but never bound."),
    // ---- section ----
    entry!(TYPE_CLOSURE_FREE_VAR, "Closure references a free variable that is not declared."),
    entry!(TYPE_INCOMPATIBLE, "Incompatible types or unit types."),
    // ---- section ----
    entry!(UNUSED_PARAM_OR_PORT, "Parameter or port is declared but never used."),
    entry!(PORT_NEVER_USED, "Port is declared but never used in any net connection."),
    entry!(UNTYPED_PARAM, "Parameter has no inferred type."),
    // ---- section ----
    entry!(ERC_SINGLE_POINT_NET, "Single-point net: only one connection."),
    entry!(ERC_UNCONNECTED_PORT, "Unconnected port: not connected to any net."),
    entry!(ERC_MULTI_DRIVE_NET, "Multi-drive net."),
    entry!(ERC_FLOATING_NET, "Floating net."),
];
