...
//      box's rect lies inside its own frame — while the boxes declared outside
//      every partition lie inside no frame.
...
fn block_frame__frames_enclose_their_member_boxes_only() {
    ...
    // Each member box sits inside its own frame — and the boxes declared
    // outside every partition sit inside none.
    ...
    for name in ["R_top1", "R_top2"] {
        let r = rect_of(box_by_name(&graph, name));
        assert!(
            !inside(r, power) && !inside(r, trim) && !inside(r, idle),
            "{name} is declared outside every partition"
        );
    }
}
