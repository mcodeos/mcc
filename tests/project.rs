...
fn projection_audit_md_is_written() {
    ...
    // The mic layer is a leaf with one dc sub-module: its port-stub nets
    // MIC.N / MIC.P are glued to the member nets MIC.N~0 / MIC.P~0, so
    // `MIC.N + MIC.N~0 → MIC.N~0` is the only projection merge (5 → 4).
    assert!(
        md.contains("| mic | 5 | 4 |"),
