...
///
/// Measured: 11 per side. It was 3 — the `MIC` union record plus the two
/// `MIC.N~0`/`MIC.P~0` port markers — until comp inner layers started naming
/// their own boundary (§3.4 stage 2): every comp pin that crosses its layer
/// now leaves the same rule-c `kept as PortTerminal marker` record the module
/// ports always left (FLASH `_CS`/`_WP`/`_HOLD`, UC `GPIO[2]`/`AVDD09_CAP`,
/// X6 `XTAL.X1`/`XTAL.X2`, lp322dcdc `EN`), on top of the `MIC` trio. The
/// count is a reading of the board, so it moves when the board's construction
/// does; what must not move is that these records are refused on both sides.
#[test]
fn a_real_board_refuses_only_its_minted_net_records() {
    ...
    assert!(d.changes.is_empty(), "changes: {:?}", d.changes);
    assert_eq!(d.unaligned.len(), 22, "11 per side: {:?}", d.unaligned);
    assert!(
        reasons(&d).iter().all(|r| r == "no-key"),
        "{:?}",
        d.unaligned
