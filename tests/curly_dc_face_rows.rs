...
/// is routed lane by lane, where the curly Node used to be placed on lane 0
/// only: the hot lane wired (the VIN/LX hot members through the element) but
/// the device's shared GND return was silently dropped — zero explicit error,
/// just NET_PARTIAL_CONNECTION and a missing current path. `stmt.rs` now places
